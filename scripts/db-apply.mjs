 ...
 *
 * For a hosted Supabase project prefer `supabase db push`; this script exists so the
 * schema can be validated and tested without the Supabase CLI or Docker.
 */
...
try {
  if (args.has('--reset')) {
    ...
    if (args.has('--local')) await client.query('drop schema if exists auth cascade; drop schema if exists storage cascade;');
  }
  if (args.has('--local')) {
